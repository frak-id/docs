...
description: React provider component for Nexus Wallet SDK configuration
date: 2024-09-06
---
...
## Summary
The `NexusConfigProvider` is a React component that provides the Nexus Wallet SDK configuration to its child components. It should be placed at the root of your React application to ensure that all Nexus SDK hooks and components have access to the necessary configuration.
...
- `config`: An object containing the Nexus Wallet SDK configuration. The `domain` property is optional and will be automatically set to the current domain if not provided.
...
const nexusConfig = {                   // [!code focus]
  walletUrl: "https://nexus.frak.id",   // [!code focus]
  metadata: {                           // [!code focus]
...
- The configuration provided here will be accessible to all Nexus SDK hooks and components within its React subtree.

## Related
- [useNexusConfig](/nexus-sdk/api/react/useNexusConfig) - Hook to access the Nexus SDK configuration
- [NexusIFrameClientProvider](/nexus-sdk/api/react/NexusIFrameClientProvider) - Provider component for the Nexus client instance
