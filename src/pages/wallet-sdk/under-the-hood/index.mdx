...
description: How the nexus wallet work under the hood?
date: 2024-03-01
---
...
In this section, we will go through the different technologies and smart contracts used by the Nexus Wallet, and how they are working together.

## Knowledge base

Before deep diving into the Nexus Wallet, it is important to understand a few blockchain technologies.

### Account Abstraction

The nexus wallet is using the [ERC-4337](https://eips.ethereum.org/EIPS/eip-4337) standard, which is a standard for account abstraction on EVM blockchain.
...
Then, the Nexus wallet is using the [WebAuthN](https://w3c.github.io/webauthn/) standard for the authentication and the signing of the transactions.
...
In this section we will go through the smart contract related to the Smart wallet side of the Nexus Wallet.
...
Since kernel wallet support modular validation, we are mainly using two type of validator for the Nexus Wallet:
...
Since the WebAuthN standard is bounded to a specific domain, we are using two principal way of communication:
 1. An iFrame, to have a bidirectional communication flow between the Nexus Wallet and the implementing website.
...
Working this way, with a **client first** approach, provide us a few key benefits:
 ...
 - All the nexus related storage is stored on the client storage, in a secure way, and is only accessible by the nexus wallet and allowed dApps.

<Callout type="note">
    On the centralisation side, yeah you are not relying on a centralised server, but you are relying on the nexus website, **how is that different?**

    Firstly, the website hosting the Nexus wallet is a PWA, meaning that user can install it on their devices (if compatible), and enjoy it everytime (even if our website come down for whatever reason).
...
The init flow is as follows:
 ...
 2. When the iFrame is loaded, it should be loaded on the **Nexus query listener** page no UI, juste handling incoming message request.
 3. Once the nexus query listener is loaded, and have warmed up a few key storage slots, it will send a `ready` message to the dApp.
 4. Once the dApp receive the `ready` message, it can start sending message to the Nexus Wallet, and receive message from it.
...
When a user action is needed, the flow with redirection is pretty straightforward:
 ...
 2. The dApp redirect the user to the Nexus Wallet URL
 3. The Nexus Wallet handle the user input, ask him to perform the action
...
Since, some sensitive data can be shared between the dApp and the Nexus Wallet, we built our own secured communication layer.
