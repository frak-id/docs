...
You have the possibility to add **metadata** by following this page: [Full Flow](/wallet-sdk/api/actions/displayModal#full-flow).
...
In your `frak-setup.js` file, add `rewardModalConfig` object to configure the referral modal:
...
Then, add the highlighted following code snippet to listen for referral:

```tsx twoslash [frak-setup.js]
// @noErrors
// [!include ~/snippets/integration/frak-setup.js:DOMContentLoaded-referral]
```
...
8. Go back to edit `frak-setup.js` file and add the following code snippet to handle the sharing button click:
...
9. Modify `window.FrakSetup` to include `modalShare`:

```javascript
// [!code word:modalShare]
