...
analytics.subscribe('checkout_completed', async (event) => {
  const wallet = await browser.sessionStorage.getItem("frak-wallet-address");
...
In your `frak-setup.js` file, add `modalConfig` object to configure the referral modal:

```tsx twoslash [frak-setup.js]
// @noErrors
// [!include ~/snippets/integration/frak-setup.js:modal-config]
```
...
6. Paste the following code snippet into **Liquid code** field on the right:
