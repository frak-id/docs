...
description: React hook to monitor the Nexus Wallet connection status
date: 2024-09-06
---
...
## Summary
The `useWalletStatus` hook provides real-time updates on the connection status of the Nexus Wallet. It allows your application to react to changes in the wallet's connection state, including whether a user is connected and if there's an active interaction session.
...
- The hook uses a WebSocket connection to receive real-time updates from the Nexus Wallet.
...
- [useOpenSso](/nexus-sdk/api/react/useOpenSso) - Hook for opening the SSO interface to connect a wallet