...
## Summary
The `siweAuthenticate` function initiates a Sign-In with Ethereum (SIWE) authentication process through the Nexus Wallet. This allows users to authenticate using their Ethereum account, providing a secure and decentralized authentication method.

## Requirements
- An initialized `NexusClient` instance
- User must have access to the Nexus Wallet
...
In this example, we import the `siweAuthenticate` function and the Nexus client. We then call `siweAuthenticate` with the necessary SIWE parameters, including a custom statement and optional metadata for the modal. The function returns a promise that resolves with the signature and the full SIWE message.
